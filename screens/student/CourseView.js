...
const CourseView = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (userInfo && data) {
      getStudentCoursesDetails(userInfo, data.slug)
        .then((res) => {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      ></View> */}
        <View
          ...
          <View>
            ...
            <Text>{serverTimeToLocalDate(course.course.created_at)}</Text>
          </View>
        </View>
        {/* <Button
      ...
      {parseInt(course?.course?.price) > 0 && (
        <View
          style={{
            flexDirection: "row",
            justifyContent: "space-between",
          ...
          <TouchableHighlight
            onPress={() => {
              ...
              addToCart(userInfo, course.course.id)
                .then((res) => {
