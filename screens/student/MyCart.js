  ...
  return (
    <ScrollView
      ...
      {cartList &&
        cartList.map((doc, i) => (
          ...
          setDiscount={setDiscount}
            ...
            onDelete={() => {
              deleteFromCart(userInfo, doc.id)
                .then((res) => {
                  setRefresh((val) => !val);
                })
                .catch((err) => {
      ...
      <View
        ...
        <View>
          <Text>৳ {(parseInt(gradTotal)+parseInt(discount)).toFixed(2)}</Text>
          <Text>- ৳ {discount}</Text>
        </View>
      </View>
      <View
      ...
      <View
        ...
        <Text>Total :</Text>
        <Text>৳ {parseInt(gradTotal).toFixed(2)}</Text>
      </View>
      <View
        style={{
          flexDirection: "row",
        ...
        <Text
          ...
          By placing your order, you agree with our company privacy policy and
          conditions of use.
        </Text>
      </View>
      <Button
        ...
        title={click ? "Loading.." : `Pay ${gradTotal} in BDT`}
        style={{
...
const Cart = ({ data, onDelete, setGrandTotal ,setDiscount}) => {
      ...
      {!ok && (
        <View
          ...
          <Button disabled={ok}
            onPress={() => {
              if (!coupon) {
                Alert.alert("Opp!", "You need to give the coupon also");
                return;
              }
              setOk(true);
              console.log(data.id)
              applyCoupon(userInfo, data.id, coupon).then((res) => {
                if (!res.data.discount ||!res.data.grand_total) {
                ...
                setCoupon()
                //setOk(true);
                setDiscount(val=>val+res.data.discount)
